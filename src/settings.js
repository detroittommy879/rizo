...
export function openSettings() {
  ...
  dialog.querySelectorAll('.stab').forEach(btn => {
    btn.addEventListener('click', () => {
      dialog.querySelectorAll('.stab').forEach(b => b.classList.remove('active'));
      dialog.querySelectorAll('.stab-content').forEach(c => c.classList.remove('active'));
  ...
  dialog.querySelector('#use-gradient')?.addEventListener('change', (e) => {
    dialog.querySelector('#gradient-options').classList.toggle('hidden', !e.target.checked);
  });
...
function buildSettingsHTML(config) {
  ...
  const fontOptions = GOOGLE_MONO_FONTS.map(f =>
    `<option value="${f}" ${f === t.googleFont ? 'selected' : ''}>${f || '(None - use custom)'}</option>`
  ).join('');

  const ansiColorInputs = ANSI_COLOR_NAMES.map(([key, label]) =>
    `<div class="color-row">
      <label>${label}</label>
...
function applySettingsFromDialog(dialog) {
  ...
  t.gradientAngle = parseInt(dialog.querySelector('#gradient-angle').value) || 135;

  // Window
  config.window.defaultCols = parseInt(dialog.querySelector('#default-cols').value) || 120;
  config.window.defaultRows = parseInt(dialog.querySelector('#default-rows').value) || 30;
